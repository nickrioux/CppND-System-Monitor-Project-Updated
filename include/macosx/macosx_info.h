...
class ProcessMacOSX {
    public:
      ...
      ProcessMacOSX(struct kinfo_proc procInfo, long ram, long activeJiffies) : pid_(procInfo.kp_proc.p_pid), ram_(ram), activeJiffies_(activeJiffies), procInfo_(procInfo) {};
      struct kinfo_proc & GetProcInfo() const { return(const_cast<struct kinfo_proc &>(procInfo_));}
...
private:
  ...
  //Process Info
  long iRunningProcess{0};
  std::set<ProcessMacOSX> processSet{};
